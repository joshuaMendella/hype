...
export default function GrowthTimeline() {
  ...
  return (
    <div ref={ref} className="rounded-3xl border border-white/10 bg-white/[0.02] p-4 sm:p-6">
      ...
      <div className="mx-auto mt-4 max-w-md">
        <div className="mb-2 flex items-center justify-between text-xs text-white/50">
          <span>Week 1</span>
          <span className="font-display text-sm text-[#4ade80]">Week {week}</span>
          <span>Week 8</span>
        </div>
        <input
          type="range"
          ...
          className="h-1.5 w-full cursor-pointer appearance-none rounded-full bg-white/15 accent-[#4ade80]"
        />
      </div>
    </div>
  )
