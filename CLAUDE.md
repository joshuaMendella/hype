...
13. **Session 23 onboarding redesign — built, LIVE DRIVE PENDING:** new-user onboarding is now a **deterministic 7-beat flow inside `ChatPanel`** (onboarding mode gated on `onboarded === false`), no LLM for beats 1–6. Beats: welcome → how-to → value+consent (translucent `ExampleConsentCard` with ghosted Yes/Not now) → ask location (seeds a `place` node + `base_profile.home_location`) → ask work/study (seeds an `org` node) → confirm (fires after nodes animate in) → **handoff:** flips `onboarded=true` client-side then replays the Q&A to `/api/chat` so the real interviewer produces a context-aware beat 7. Nodes seeded client-side via browser Supabase/RLS (`lib/onboarding/seed.ts`); copy in `lib/onboarding/script.ts`; both You-edges are GraphCanvas synthetic `self` links (place needs its title in live `identityPlaces` state, lifted into `GraphWrapper`; org auto-links). **This deleted the LLM onboarding path from `route.ts` — the permanent fix for the Gemini degeneration bug** (see [[project_gemini_degeneration]]). Owner live drive: `/hype-reset` → walk all 7 beats, confirm two nodes pop + link to You, confirm `onboarded=true` persists on reload. Known edge (minor, self-recovers): ~900ms window on the confirm beat where a stray Enter briefly blanks the line before the handoff timer replaces it. Spec: `docs/superpowers/specs/2026-07-12-onboarding-redesign-design.md`; plan: `docs/superpowers/plans/2026-07-12-onboarding-redesign.md`.
...
│   ├── onboarding/                 ← deterministic new-user flow: script.ts (fixed beat copy), seed.ts (client RLS seeding of place+org nodes, home_location merge, completeOnboarding) — check: scripts/check-onboarding-seed.ts
