...
14. **Admin roadmap tracker (session 25)** — `/admin/roadmap` renders `apps/web/lib/admin/roadmap.ts` (10 areas, each with a `principle` + status items + launch-blocker flags). **Keep that file updated whenever session work changes an item's state — same discipline as CLAUDE.md.** Owner still owes: eyeball the page + correct the Marketing & socials placeholder items.
...
│   ├── (admin)/admin/              ← owner-only (ADMIN_USER_ID gate, fails closed 404; layout AND page both check): page.tsx dashboard (users + events) + roadmap/page.tsx build tracker
...
│   ├── admin/roadmap.ts            ← build-tracker source of truth (10 areas × principle + status items, launchBlocker flags) rendered at /admin/roadmap — UPDATE EACH SESSION like CLAUDE.md; sanity check: scripts/check-roadmap.ts
